...
bool JlCompress::compressSubDir(QuaZip* zip, QString dir, QString origDir, bool recursive, QDir::Filters filters) 
{
    ...
    // Checking the opening of the zip
    if (!zip) {
        #ifdef Q_DEBUG
    ...
    if (!directory.exists()) {
        #ifdef Q_DEBUG
            qDebug() << "*** JlCompress::compressSubDir() - Fatal error: folder doesn't exist -> " << dir;
        #endif
        return false;
    }
    ...
    // If I also compress the sub folders
    if (recursive) {
        // For each sub-folder
    ...
    for (int index = 0; index < files.size(); ++index) {
        const QFileInfo & file(files.at(index));
        // If it's not a file or it's the compressed file being processed
        if (!file.isFile() || file.absoluteFilePath() == zip->getZipName()) 
            continue;

        // Creating the relative name to use within the compressed file
        QString filename = origDirectory.dirName() + QLatin1String("/") + origDirectory.relativeFilePath(file.absoluteFilePath());

        #ifdef Q_DEBUG
            qDebug() << "*** JlCompress::compressSubDir() - Adding file -> " << filename;
        #endif
